...
export const handleCancelEmptySimUK = async (
    recordId: string,
    orderId: string | undefined,
    setErrorMessage: (msg: string) => void,
    setShowErrorModal: (show: boolean) => void,
    ...
    try {
        ...
        const response = await fetch('https://cancelSimcardUK-ezeznlhr5a-uc.a.run.app', {
            method: 'POST',
            headers: {
                'authorization': `${idToken}`,
                'Content-Type': 'application/json'
        ...
        if (response.ok && data.success) {
            ...
            if (data.message === 'Unauthorized') {
                ...
                errorMsg = 'Please refresh the page and try again';
            } else if (data.message === 'Number cannot be cancelled' || data.message === 'Missing orderId in request body') {
                errorMsg = 'This number could not be cancelled, please try again or contact our customer support';
            } else if (data.message === 'Internal Server Error') {
                errorMsg = 'Please contact our customer support';
            }  else if (data.message === 'You must wait 5 minutes after purchase before cancelling') {
                errorMsg = 'You must wait 5 minutes before cancelling';
            }
...
export const handleActivateEmptySimUK = async (
    recordId: string,
    orderId: string | undefined,
    setErrorMessage: (msg: string) => void,
    setShowErrorModal: (show: boolean) => void,
    ...
    try {
        ...
        const response = await fetch('https://activateemptysimcarduk-ezeznlhr5a-uc.a.run.app', {
            method: 'POST',
            headers: {
                'authorization': `${idToken}`,
                'Content-Type': 'application/json'
        ...
        if (response.ok && data.success) {
            ...
            if (data.message === 'Unauthorized') {
                ...
                errorMsg = 'Please refresh the page and try again';
            } else if (data.message === 'Number cannot be activated' || data.message === 'Missing orderId in request body') {
                errorMsg = 'This number could not be activated, please try again or contact our customer support';
            } else if (data.message === 'Internal Server Error') {
                errorMsg = 'Please contact our customer support';
            }
