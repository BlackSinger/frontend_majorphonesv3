...
const SendSMS: React.FC = () => {
    ...
    const [searchResult, setSearchResult] = useState<CountryData | null>(null);
    ...
    // Buscar país en Firestore por nombre
    const handleSearchCountry = async () => {
        const inputText = countryFilter.trim().toLowerCase();
        if (!inputText) return;

        // Buscar en countries_simple.json la coincidencia
        const match = countriesSimple.find(
            (c: any) => c.country.toLowerCase().startsWith(inputText)
        );
        ...
        // Buscar el documento en Firestore por isoCountry
        setSearching(true);
        try {
            const docRef = doc(db, 'sendSMS', 'opt1', 'countries', match.isoCountry);
            ...
            if (docSnap.exists()) {
                ...
                setError('Country data not found in database.');
                setShowErrorModal(true);
    ...
    // Cuando el usuario modifica el input, limpiar el resultado de Firestore
    const handleFilterChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        setCountryFilter(e.target.value);
        setSearchResult(null); // Vuelve a la tabla normal
    };
        ...
        <div className="space-y-6">
            ...
            <div className="rounded-3xl shadow-2xl border border-slate-700/50 p-6 relative overflow-hidden">
                <div className="relative z-10">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            ...
                            <tbody>
                                {loading || searching ? (
                                    <tr>
                                        <td colSpan={searchResult ? 3 : 2} className="px-6 py-20">
                                    ...
                                    <tr className="border-b border-slate-700/50 hover:bg-slate-800/30 transition-colors duration-200">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center space-x-3">
                                                {getFlagEmoji(searchResult.isoCountry)}
                                                <span className="text-white">{searchResult.country}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className="text-slate-300">+{searchResult.areaCode}</span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className="text-emerald-400 font-semibold">
                                                ${searchResult.maxPrice.toFixed(2)}
                                            </span>
                                        </td>
                                    </tr>
                                ) : countries.length === 0 ? (
                                    <tr>
                                        <td colSpan={searchResult ? 3 : 2} className="px-6 py-20 text-center">
                    ...
                    {/* Pagination */}
                    {!searchResult && !searching && filteredCountries.length > 0 && totalPages > 1 && (
                        <div className="mt-6">
