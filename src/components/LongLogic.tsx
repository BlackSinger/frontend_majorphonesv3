...
const loadActivationTimestamps = (): { [orderId: string]: number } => {
  try {
    const stored = localStorage.getItem('longActivationTimestamps');
...
const saveActivationTimestamps = () => {
  try {
    localStorage.setItem('longActivationTimestamps', JSON.stringify(activationTimestamps));
  } catch (error) {
    console.error('Error saving activation timestamps:', error);
  }
};

// Function to check if a Long number has timed out (for display purposes only)
export const hasLongTimedOut = (record: HistoryRecord): boolean => {
  const recordId = record.orderId || record.id;

  // If status is not Active, clear timestamp and return false
  if (record.serviceType !== 'Long' || record.status !== 'Active') {
...
// Get remaining time for Long countdown (returns null if not applicable, or time string like "2:32")
export const getLongCountdownTime = (record: HistoryRecord): string | null => {
...
// Get display status for Long numbers
export const getLongDisplayStatus = (record: HistoryRecord): string => {
  // Show fictitious "Inactive" when status is Active but timed out (3 min) without new SMS
  if (hasLongTimedOut(record)) {
    return 'Inactive';
  }
...
export const getLongAvailableActions = (record: HistoryRecord): string[] => {
  ...
  const displayStatus = getLongDisplayStatus(record);
...
export const handleCancelLong = async (
  ...
  try {
    ...
    // Make API call to cancel long number cloud function
    const response = await fetch('https://cancellongusa-ezeznlhr5a-uc.a.run.app', {
      method: 'POST',
      headers: {
    ...
    if (response.ok && data.success) {
      ...
      if (data.message === 'Unauthorized') {
        ...
        errorMsg = 'Please refresh the page and try again';
      } else if (data.message === 'Number cannot be cancelled' || data.message === 'Error cancelling long') {
...
export const handleActivateLong = async (
  ...
  try {
    ...
    // Make API call to activate long number cloud function
    const response = await fetch('https://activatelongusa-ezeznlhr5a-uc.a.run.app', {
      method: 'POST',
      headers: {
        'authorization': `${idToken}`,
        'Content-Type': 'application/json'
