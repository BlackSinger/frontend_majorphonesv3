...
export const handleReuseNumber = async (
  ...
  try {
    ...
    const response = await fetch('https://reusenumber-ezeznlhr5a-uc.a.run.app', {
      method: 'POST',
      headers: {
        'authorization': `${idToken}`,
        'Content-Type': 'application/json'
    ...
    if (response.ok && data.success) {
      ...
      if (data.message === 'Unauthorized') {
        ...
        errorMsg = 'Please refresh the page and try again';
      } else if (data.message === 'This number is not reusable') {
        errorMsg = 'This number cannot be reused';
      } else if (data.message === 'Number is still pending') {
        errorMsg = 'Please wait until the number is completed before reusing';
      } else if (data.message === 'Error reusing number') {
