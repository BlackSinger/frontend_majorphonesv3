...
const MajorTickets: React.FC = () => {
  ...
  const formatDate = (timestamp: any): string => {
    ...
    try {
      ...
      if (timestamp._seconds !== undefined) {
        // Handle Firebase Timestamp format from backend
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      ...
      try {
        ...
        console.log('Number of tickets received:', data.orders?.length);

        if (data.orders && Array.isArray(data.orders)) {
          console.log('Processing tickets...');
          const tickets: TicketRecord[] = data.orders.map((ticket: any) => ({
            id: ticket.ticketId || ticket.id,
            ticketId: ticket.ticketId || ticket.id,
            date: formatDate(ticket.createdAt),
            lastUpdate: formatDate(ticket.updatedAt),
            issue: mapTypeToDisplayName(String(ticket.type || '')),
            subject: ticket.subject || '',
            status: (String(ticket.status || 'open').toLowerCase() === 'closed' ? 'closed' : 'open') as 'open' | 'closed',
            message: ticket.message || '',
            images: ticket.images || [],
            response: ticket.response || [],
            createdAt: ticket.createdAt,
            updatedAt: ticket.updatedAt,
            type: String(ticket.type || '').toLowerCase(),
            email: ticket.email || ''
          }));
          ...
          tickets.sort((a, b) => {
            ...
            if (a.createdAt?._seconds) {
              aTime = a.createdAt._seconds * 1000;
            ...
            if (b.createdAt?._seconds) {
              bTime = b.createdAt._seconds * 1000;
          ...
          console.log('Setting ticket data with', tickets.length, 'tickets');
          setTicketData(tickets);
        } else {
          console.log('No valid tickets data received');
          setTicketData([]);
        }
        setIsLoading(false);
      } catch (error) {
  ...
  const handleCloseTicket = async (ticketId: string) => {
    ...
    try {
      ...
      const response = await fetch('https://changestatus-ezeznlhr5a-uc.a.run.app', {
        method: 'POST',
        headers: {
          'authorization': `${idToken}`,
          'Content-Type': 'application/json'
        ...
        setTicketData(prevTickets =>
          prevTickets.map(ticket =>
            ticket.ticketId === ticketId
              ? { ...ticket, status: 'closed' as 'open' | 'closed' }
              : ticket
          )
        );
          ...
          <div className="relative z-10">
            <div className="flex items-center space-x-4">
              <div>
                ...
                <p className="text-slate-300 text-md text-left">View and manage all tickets</p>
              </div>
            </div>
          </div>
        </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-700/50">
                      ...
                      <th className="text-center py-4 px-6 text-slate-300 font-semibold">User</th>
                      ...
                      >
                        ...
                        <td className="py-4 px-6">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            >
                              {closingTicketId === record.ticketId ? (
                  ...
                  <div className="flex flex-col h-[500px] sm:h-96">
                      ...
                      <div className="relative bg-slate-700/50 rounded-lg px-4 py-2 max-w-[85%] sm:max-w-sm md:max-w-md">
                        ...
                        {selectedTicket.images && selectedTicket.images.length > 0 && (
                          <div className="mt-2 space-y-1">
                            <div className="text-xs mb-1 text-slate-200">
                            ...
                            {selectedTicket.images.map((image, imgIndex) => (
                              <div
                                ...
                                className="flex items-center space-x-2 rounded px-2 py-1 cursor-pointer hover:opacity-80 transition-opacity bg-slate-600/30"
                              >
                                <svg className="w-3 h-3 flex-shrink-0 text-slate-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                </svg>
                                <span className="text-xs truncate text-slate-200">{image.fileName}</span>
                              </div>
                            ))}
                          </div>
                        )}
                    ...
                    <div className="flex-1 overflow-y-auto mb-4 space-y-3 px-3">
                      ...
                      {selectedTicket.response && selectedTicket.response.length > 0 && selectedTicket.response.map((response, index) => (
                        <div key={`response-${index}`} className={`flex ${response.isUser ? 'justify-start' : 'justify-end'}`}>
                          <div className={`relative rounded-lg px-4 py-2 max-w-[85%] sm:max-w-sm md:max-w-md ${
                            ...
                            {response.images && response.images.length > 0 && (
                              <div className="mt-2 space-y-1">
                                <div className={`text-xs mb-1 ${response.isUser ? 'text-slate-200' : 'text-emerald-200'}`}>
                                ...
                                {response.images.map((image, imgIndex) => (
                                  <div
                                    ...
                                    className={`flex items-center space-x-2 rounded px-2 py-1 cursor-pointer hover:opacity-80 transition-opacity ${
                                      response.isUser ? 'bg-slate-600/30' : 'bg-emerald-700/30'
                                    }`}
                                  >
                                    <svg className={`w-3 h-3 flex-shrink-0 ${response.isUser ? 'text-slate-200' : 'text-emerald-200'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                    </svg>
                                    <span className={`text-xs truncate ${response.isUser ? 'text-slate-200' : 'text-emerald-200'}`}>{image.fileName}</span>
                                  </div>
                                ))}
                              </div>
                            )}
                      ...
                      {/* New messages from admin (sent during this session) */}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" style={{ margin: '0' }}>
            <div className="bg-white/10 backdrop-blur-xl rounded-2xl shadow-2xl p-6 w-96">
              <div className="text-center">
                ...
                <div className="text-center mb-6">
                  <p className="text-slate-300">This ticket has been closed</p>
                </div>
                <div className="flex justify-center">
                  <button
